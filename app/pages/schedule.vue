...
const formatDate = (walkDate: string) => {
  const newDate = new Date(walkDate);
  return newDate.toLocaleDateString("en-GB", { weekday: "short", year: "numeric", month: "short", day: "numeric" });
};
</script>
  ...
  <div class="container py-4">
    <div class="flex justify-center">
      <table class="w-1/2">
        ...
        <tbody>
          <tr v-for="(walk, index) in sortedWalks" :key="`walk-${index}`" :class="[index % 2 ? '' : evenRowClass]">
            <td class="text-center">{{ formatDate(walk.date) }}</td>
            <td class="text-center">{{ walk.location }}</td>
            <td class="text-center">{{ walk.postcode }}</td>
            <td class="text-center">{{ walk?.organiser?.name }}</td>
            <td class="text-center">{{ walk.contact }}</td>
