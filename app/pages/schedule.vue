...
<template>
    ...
    <div class="flex justify-center">
      <table class="w-full rounded bg-blue-50">
        <thead>
          <tr class="lg:text-lg">
            <th>Date</th>
            <th>Location</th>
            <th>Postcode</th>
            <th class="hidden lg:table-cell">Leader</th>
            <th>Contact</th>
        ...
        <tbody>
          <tr v-for="(walk, index) in sortedWalks" :key="`walk-${index}`" :class="[index % 2 ? '' : 'bg-white']">
            ...
            <td class="text-center hidden lg:block">{{ walk?.organiser?.name }}</td>
